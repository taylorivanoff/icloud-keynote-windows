...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({ width: 1920, height: 1080, webPreferences: { nodeIntegration: true, partition: 'persist:icloud-keynote' } });
  ...
  mainWindow.loadURL('https://www.icloud.com/keynote');
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip('iCloud Keynote');
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Show iCloud Keynote', click: () => { if (!mainWindow) createWindow(); mainWindow.show(); } },
...
app.on('before-quit', async () => { await session.fromPartition('persist:icloud-keynote').cookies.flushStore(); });
